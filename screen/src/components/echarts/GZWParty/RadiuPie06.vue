...
-->
<template>
  <div class="RadiuPieSixth" ref="RadiuPieSixth"></div>
</template>
<script>
...
export default {
  ...
  methods: {
    drawLineCustom(obj) {
      var myChart = this.$echarts.init(this.$refs.RadiuPieSixth)
      let option = {
        title: {
        ...
        color: ['#0F9590', '#61B879', '#F4B31D', '#5DB3E6', '#FF5523'],
        legend: {
          show: false,
        },
        series: [
          {
            ...
            data: obj.data
              ? obj.data
              : [
                { value: 4166, name: '年度投资计划' },
                { value: 2017, name: '重大工程建设' },
