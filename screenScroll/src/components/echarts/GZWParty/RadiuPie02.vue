...
-->
<template>
  <div class="RadiuPieSecond" ref="RadiuPieSecond"></div>
</template>
<script>
...
export default {
  ...
  methods: {
    drawLineCustom(obj) {
      var myChart = this.$echarts.init(this.$refs.RadiuPieSecond)
      let option = {
        title: {
          text: '党员概况', // 图形标题，配置在中间对应效果图的80%
          left: 'center',
          top: '45%',
          textStyle: {
        ...
        color: ['#FF7B50', '#FAC858', '#92CC76', '#5470C6'],
        legend: {
          show: true,
          x: 'center',
          padding: [320, 0, 0, 0],
        ...
        series: [
          {
            ...
            data: obj.data
              ? obj.data
              : [
                { value: 4166, name: '正式党员' },
                { value: 2017, name: '预备发展对象' },
                { value: 2017, name: '申请入党' },
                { value: 2017, name: '入党积极分子' },
              ],
          },
        ],
      }
